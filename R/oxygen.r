...
#' @note `pandoc` 及 `ConTeXt` 已在系统路径中。`ConTeXt` 环境初始化命令为 `setuptex2.bat`。 
#' @param fn 待转换的文件名
...
md2context<-function(fn, template='r_pkg', toPdf = TRUE){
  #  basename.noext <- function(path){
  #    ret = basename(path)
  #    unlist(lapply(strsplit(ret, split='\\.'), `[`, 1L))
  #  } 
  ...
  # make pdf
  if (toPdf) {
    system(paste('setuptex2.bat && context ', fn_tex))
    if (file.exists(fn_pdf)) file.copy(fn_pdf, file.path(dirname(fn), 'inst/doc/', fn_pdf),
                                       recursive = TRUE)
  }
}
